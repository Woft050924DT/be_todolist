...
exports.initializeTaskSchema = initializeTaskSchema;
...
async function initializeTaskSchema() {
  ...
  `);
    ...
    if (missingColumns.length > 0) {
        throw new Error(`dbo.Tasks schema is missing column(s): ${missingColumns.join(", ")}`);
    }
}
class TaskService {
    async get(filters, res) {
        try {
            const request = (0, dbConfig_1.getPool)().request();
            let query = "SELECT * FROM dbo.Tasks WHERE 1=1";
            if (filters.categoryId) {
                request.input("CategoryId", mssql_1.default.Int, filters.categoryId);
                query += " AND CategoryId = @CategoryId";
            }
            if (filters.status !== undefined) {
                request.input("Status", mssql_1.default.TinyInt, filters.status);
                query += " AND Status = @Status";
            }
            if (filters.priority !== undefined) {
                request.input("Priority", mssql_1.default.TinyInt, filters.priority);
                query += " AND Priority = @Priority";
            }
            if (filters.dueDateFrom) {
                request.input("DueDateFrom", mssql_1.default.DateTime2, filters.dueDateFrom);
                query += " AND DueDate >= @DueDateFrom";
            }
            if (filters.dueDateTo) {
                request.input("DueDateTo", mssql_1.default.DateTime2, filters.dueDateTo);
                query += " AND DueDate <= @DueDateTo";
            }
            if (filters.search) {
                request.input("Search", mssql_1.default.NVarChar(255), `%${filters.search}%`);
                query += " AND (Title LIKE @Search OR Description LIKE @Search)";
            }
            query += " ORDER BY TaskId DESC";
            const result = await request.query(query);
            if (filters.includeRelated) {
                const enrichedTasks = await this.attachRelatedData(result.recordset);
    ...
    async create(task, res) {
        const data = normalizeTask(task);
        try {
            await (0, dbConfig_1.getPool)()
                .request()
                .input("CategoryId", mssql_1.default.Int, data.CategoryId)
                .input("Title", mssql_1.default.NVarChar(255), data.Title)
                .input("Description", mssql_1.default.NVarChar(mssql_1.default.MAX), data.Description)
                .input("Priority", mssql_1.default.TinyInt, data.Priority)
                .input("Status", mssql_1.default.TinyInt, data.Status)
                .input("DueDate", mssql_1.default.DateTime2, data.DueDate ? new Date(data.DueDate) : null)
                .query(`
    ...
    async attachRelatedData(tasks) {
        if (!tasks || tasks.length === 0) {
            return tasks;
        }
        const taskIds = tasks.map(t => t.taskId);
        const categoryIds = tasks.filter(t => t.categoryId).map(t => t.categoryId).filter(Boolean);
        ...
        tags.forEach(t => {
            if (!tagsMap.has(t.taskId))
                tagsMap.set(t.taskId, []);
    ...
    async getCategories(categoryIds) {
        if (categoryIds.length === 0)
            return [];
        const result = await (0, dbConfig_1.getPool)().request()
            .input("CategoryIds", mssql_1.default.VarChar, categoryIds.join(','))
            .query(`SELECT * FROM dbo.Categories WHERE CategoryId IN (SELECT value FROM STRING_SPLIT(@CategoryIds, ','))`);
        return result.recordset;
    }
    async getSubTasks(taskIds) {
        if (taskIds.length === 0)
            return [];
        const result = await (0, dbConfig_1.getPool)().request()
            .input("TaskIds", mssql_1.default.VarChar, taskIds.join(','))
